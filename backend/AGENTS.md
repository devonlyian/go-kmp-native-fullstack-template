...
- After native app flow validation, the app scope drafts the needed SDL from demonstrated data requirements. Review its contract handoff for implementability, nullability, and failure semantics, then settle the contract with app before backend implementation. Treat provisional app fixtures as examples, not agreed server behavior; flag impossible or ambiguous requirements and return product/UX changes to planning/design. A contract-only review does not start server implementation.
