...
| `backend/internal/graphql` | 공유 GraphQL transport: gqlgen 생성 코드와 스키마별 resolver 파일. 각 도메인의 공개 Application API에 위임 |
...
GraphQL 계약은 `contracts/graphql/`의 도메인별 SDL 파일로 관리하고 하나의 공개 API를 구성한다. `backend/graph/schema`를 두 번째 원본으로 추가하지 않는다. gqlgen은 모든 SDL 파일을 하나의 스키마와 하나의 resolver root로 합치므로 생성 코드는 공유 `internal/graphql` 패키지에 두고, 새 `<domain>.graphqls`는 해당 도메인의 Application에 위임하는 `internal/graphql/<domain>.resolvers.go`를 만든다. 다른 도메인의 로직을 `system`에 몰아넣지 않는다.
