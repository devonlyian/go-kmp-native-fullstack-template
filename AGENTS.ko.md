...
- `contracts/graphql/`이 유일한 공통 API 기준이다. 구현 전에 입력·출력·nullability·실패 의미를 정하고 동작은 SDL 설명으로 정의한다. 부족한 정보는 상대 코드를 역추적할 이유가 아니라 계약의 빈 부분이다. 필요한 계약 변경과 인계는 따로 처리하며 양쪽 구현을 몰래 함께 바꾸지 않는다. 필요할 때만 [계약 경계](docs/architecture.ko.md)를 읽는다.
