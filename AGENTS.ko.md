...
- 새 기능 순서는 승인된 기획 → ui-ux-pro-max 방향 시드(선택) → Figma MCP로 Figma에 직접 디자인 → 승인된 파일 기반 Design System → 디자인 토큰 매칭 → 네이티브 앱 개발 → GraphQL 계약 생성 → 백엔드 개발이다. [작업 흐름과 계약 경계](docs/architecture.ko.md)를 따르며 기존 기능은 승인된 산출물을 재사용하고 요청된 단계부터 진행한다.
