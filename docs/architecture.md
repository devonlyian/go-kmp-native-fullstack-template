...
Backend and app are independent development scopes. Their shared API agreement is the single SDL source at `contracts/graphql/`, including descriptions of behavior. Implementation details stay in their own scopes: [backend](../backend/docs/architecture.md) or [app](../app/docs/architecture.md). Read only the relevant one.
...
2. If an input, field, nullability rule, or failure behavior is missing, record the exact contract gap and proposed change. Settle the contract before dependent implementation; use SDL descriptions for semantics that types cannot express. Do not invent server behavior in an app Mock.
