...
5. For a material choice that changes scope, ask one concise decision before finalizing. Otherwise keep minor uncertainty visible in the plan. After approval, hand flows, states, content, and platform constraints to design for direct Figma design, Design System creation from the approved file, and token matching. Preserve actions, data concepts, outcomes, and failure semantics for subsequent app validation and then contract creation; do not propose SDL fields or start contract work from planning.
