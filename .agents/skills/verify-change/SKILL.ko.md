...
앱 UI 변경은 승인된 Design System·토큰 대응을 기준으로 관련 플랫폼 테스트와 실제 화면·접근성 검토가 필요하다. 기능 계약 전에는 임시임을 명시한 로컬 fixture·테스트 대역으로 동작하는 네이티브 흐름을 검증하며 계약 부재가 이 단계를 막지 않는다. 합의 뒤 모델·fixture 정합성과 Apollo 생성을 검사하고 계약에 맞는 Mock이나 제공된 API를 사용한다. 임시 fixture·계약에 맞춘 Mock·실제 API 증거를 구분해 보고한다. 기본 스크립트는 Android 연결 테스트·KMP iOS 테스트·네이티브 iOS 앱/UI 테스트·화면 검토를 실행하지 않는다. Mock은 실제 서버 호환성을 증명하지 않으며 백엔드 완성 뒤 앱 연결·요청된 통합 검사가 남는다.
