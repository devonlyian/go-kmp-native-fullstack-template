...
2. Specify backend behavior using [the backend template](../../../backend/docs/features/_template.md) for a new feature. Reuse the contract; if incomplete, identify and settle the contract change before implementation. Follow [contract handoff](../../../docs/architecture.md) only for that case. Never infer requirements from the app.
