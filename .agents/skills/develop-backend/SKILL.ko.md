...
1. [백엔드 규칙](../../../backend/AGENTS.ko.md), 관련 `backend/docs/features/` 메모와 루트 `contracts/graphql/`을 읽는다. 문서 쌍 관리나 명시적 요청 외에는 영어를 읽는다. app 코드·앱 안내·영역 간 기능 기록은 불러오지 않는다.
2. 새 기능은 [백엔드 양식](../../../backend/docs/features/_template.ko.md)으로 서버 동작을 정한다. 앱 범위가 승인된 디자인의 데이터 요구사항으로 SDL 변경 draft를 작성하며, 구현 가능성·nullability·실패 의미를 검토하고 구현 전에 계약을 확정한다. 그때만 [계약 인계](../../../docs/architecture.ko.md)를 따른다. 앱에서 요구를 추측하지 않는다.
