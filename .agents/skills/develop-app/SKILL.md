...
2. For a new feature use [the app template](../../../app/docs/features/_template.md). Record flow, loading/error/retry/cancellation, platforms, and acceptance criteria. Reuse the existing contract. When fields or semantics are missing, draft the SDL change from the approved design's data requirements; the backend scope reviews it for implementability. Never implement the server or guess responses.
...
4. Update app-owned GraphQL Operations and regenerate Apollo via [app checks](../../../app/docs/checks.md). Screens and ViewModels may be scaffolded from the approved design's data concepts before the SDL is settled; wire repositories only against the confirmed contract. Implement Shared Domain/Data/Network and the requested native platforms. Use contract-aligned repository/HTTP Mocks when no service is available; this does not require adding a Mock server or starting the backend.
