...
1. [앱 규칙](../../../app/AGENTS.ko.md)과 관련 `app/docs/features/` 메모를 읽고 UI 작업에는 승인된 디자인·Design System·토큰 인계도 읽는다. 기존 작업은 요청된 단계부터 진행하며 승인된 Figma나 합의된 로컬 기준을 Make에서 다시 만들지 않고 재사용한다. 문서 쌍 관리나 명시적 요청 외에는 영어를 사용한다. API 계약을 재사용하거나 만들 때 루트 `contracts/graphql/`을 읽으며 기능 계약이 없어도 네이티브 앱 개발은 진행한다. 백엔드 코드·SQL·서버 안내는 읽지 않는다. 공통 API 기록은 요청된 계약 인계 때만 읽는다.
2. 새 기능은 [앱 양식](../../../app/docs/features/_template.ko.md)을 사용한다. 흐름·로딩/오류/재시도/취소·플랫폼·완료 조건을 기록한다. UI 작업은 승인된 Make 기반 디자인·기존 승인 Figma·합의된 로컬 명세를 확정된 Design System·토큰 대응에 따라 네이티브 Compose·SwiftUI로 구현한다. 기존 컴포넌트를 재사용하고 계획된 네이티브 심볼은 구현한 뒤 실제 매핑 예외에 기록한다. 새 디자인 결정은 [design-app](../design-app/SKILL.ko.md)에 전달한다. Figma 생성·수정은 요청된 경우에만 하며 작업을 포함하는 기존 승인을 재사용한다. 승인된 기준이 없으면 화면 일치를 주장하지 않는다.
