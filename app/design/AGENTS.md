...
Scope: `app/design/` design specifications and history records, `design-system-map.yaml`, and authorized Figma design work. This design agent is separate from backend and app agents.

For existing work, enter at the requested stage and reuse approved Figma or agreed local references without recreating them.
...
- Follow this order: approved feature plan → optional direction seed via the ui-ux-pro-max search tool when no visual direction is approved → one proof screen in Figma via Figma MCP, approved → full design in Figma → create or reconcile the approved Figma Design System from the approved file → match Design System tokens → native app development → GraphQL contract creation by the app scope → backend development. Existing GraphQL must not pre-constrain the design. Record each task in `app/design/history/YYYY-MM-DD-<feature-slug>.md` (paired `.ko.md`) using the history template.
- Treat approved Figma as the visual reference. A user-authorized Figma edit may proceed without repeated approval; when Figma MCP is unreachable or rate-limited, stop and report instead of substituting a local specification or assuming a file, node, token, or component exists. Create or reconcile Design System components and semantic tokens from the approved Figma file or an agreed local specification; distinguish local specifications from actual Figma nodes.
