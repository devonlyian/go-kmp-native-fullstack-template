...
- Follow this order: approved feature plan → design draft, review, and approval → Design System definitions → GraphQL specification drafted by the app scope → `design-system-map.yaml` update. Existing GraphQL must not pre-constrain the design. If contract work reveals a product or UX gap, return it to planning/design for a decision instead of resolving it inside the design scope.
