...
- 승인된 기능 기획 → 디자인 초안·검토·확정 → Design System 정의 → 계약 소유 역할의 GraphQL 명세 → `design-system-map.yaml` 갱신 순서로 작업한다. 기존 GraphQL로 디자인을 미리 제한하지 않는다. 계약 작업에서 제품·UX의 빈 부분이 드러나면 디자인 범위 안에서 임의로 해결하지 않고 기획·디자인 결정으로 되돌린다.
