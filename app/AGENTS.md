...
Scope: `app/` (Shared, Android, iOS), its documentation, app tools/configuration, and root `contracts/graphql/`. Do not read Go resolvers, repositories, DB schema, or backend docs to infer app behavior. Consume the SDL and its descriptions; report contract gaps instead of guessing or implementing the server.
...
- Apollo reads root SDL directly. Update only app-owned Operations and generate client types; generated Kotlin stays in build outputs. A backend implementation or DB change is not part of app work.
- Develop against contract-aligned responses/Mocks when no API is available; model data, GraphQL/transport errors, nullability, and cancellation. Use an existing API endpoint for connection checks when available. Mocks do not prove server compatibility; do not start backend work just to unblock UI.
