...
- UI는 승인된 Figma 파일·합의된 로컬 명세 중 해당 원본과 Design System 참조를 사용하며 네이티브 컴포넌트와 `design/design-system-map.yaml` Token을 재사용한다. 토큰 매칭은 앱 개발보다 앞서며 GraphQL 승인이 필요하지 않다. 계획된 네이티브 심볼은 구현한 뒤 실제 매핑 예외에 기록한다. 의미 기반 색·타이포그래피·간격·반경은 Design System에 둔다.
